...
export function MemberHome({ user, onLogout }) {
  ...
  return <div className="member-shell"><header className="member-header"><strong><img src="/assets/qismat-connections-logo.png" alt="Qismat Connections" /></strong><nav className="member-nav" aria-label="Member navigation"><button className={view === 'home' ? 'active' : ''} onClick={() => { setView('home'); setEditing(false); }}>Home</button><button className={view === 'discover' ? 'active' : ''} onClick={() => { setView('discover'); setEditing(false); }}>Discover</button><button className={view === 'saved' ? 'active' : ''} onClick={() => { setView('saved'); setEditing(false); }}>Saved</button><button className={view === 'interests' ? 'active' : ''} onClick={() => { setView('interests'); setEditing(false); }}>Interests</button><button className={view === 'messages' ? 'active' : ''} onClick={() => { setView('messages'); setEditing(false); }}>Messages</button><button className={view === 'photos' ? 'active' : ''} onClick={() => { setView('photos'); setEditing(false); }}>Photo access</button><button className={view === 'notifications' ? 'active' : ''} onClick={() => { setView('notifications'); setEditing(false); }}>Updates</button><button className={view === 'settings' ? 'active' : ''} onClick={() => { setView('settings'); setEditing(false); }}>Settings</button></nav><div><span>{user.name}</span><button onClick={onLogout}>Sign out</button></div></header>
    {view === 'discover' ? <Discovery /> : view === 'saved' ? <Discovery mode="saved" /> : view === 'interests' ? <Interests /> : view === 'messages' ? <Messages /> : view === 'photos' ? <PhotoRequests /> : view === 'notifications' ? <Notifications onNavigate={setView} /> : view === 'settings' ? <AccountSettings onSignedOut={onLogout} /> : editing ? <ProfileEditor profile={profile} preferences={preferences} status={status} onCancel={() => setEditing(false)} onComplete={completed} /> : <main className="member-main">
      <section><span className="kicker">Your membership</span><h1>Welcome, {user.name.split(' ')[0]}.</h1><p>Your secure account is connected. Complete your profile, submit it for review, and choose when approved whether to appear in discovery.</p>
        ...
        {nextAction && <div className="member-guidance"><span>Recommended next step</span><strong>{nextAction.title}</strong><p>{nextAction.description}</p>{status.missing_required_fields?.length > 0 && <small>Missing: {status.missing_required_fields.join(', ')}</small>}{nextAction.code === 'enter_discovery' ? <button className="btn btn-primary" onClick={toggleDiscovery} disabled={busy}>Enter discovery</button> : nextAction.code === 'discover_profiles' ? <button className="btn btn-primary" onClick={() => setView('discover')}>View matches</button> : !['await_review'].includes(nextAction.code) && <button className="btn btn-primary" onClick={() => setEditing(true)}>Continue profile</button>}</div>}
